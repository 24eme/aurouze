...
class CalendarController extends Controller {
    ...
    public function planifierAction(Request $request, $planifiable) {
        ...
        if ($date = $planifiable->getDateForPlanif()) {
            return $this->redirectToRoute('calendar', array('planifiable' => $planifiable->getId(),'id' => $planifiable->getEtablissement()->getId(), 'date' => $date->format('d-m-Y'), 'technicien' => $planifiable->getTechniciens()->first()->getId()));
        } else {
            return $this->redirectToRoute('calendar', array('planifiable' => $planifiable->getId(),'id' => $planifiable->getEtablissement()->getId(), 'technicien' => $planifiable->getTechniciens()->first()->getId()));
        }
    }
