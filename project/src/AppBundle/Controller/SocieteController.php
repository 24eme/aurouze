...
class SocieteController extends Controller {
    ...
    public function exportAction(Request $request) {
        ...
        $response = new StreamedResponse(function() use($dm) {
            ...
            fputcsv($out, array("Identifiant", "Type", "Statut", "Catégorie", "Raison sociale", "Nom", "Adresse", "Code postal", "Commune", "SIRET", "Code comptable", "Fréquence de paiement", "Téléphone fixe", "Téléphone mobile", "Email", "Site internet", "Libellé de contact", "Provenance"), ';');
            foreach ($societes as $societe) {
                fputcsv($out, array($societe->getIdentifiant()."", "SOCIETE", ($societe->getActif() ? "ACTIF" : "SUSPENDU"), $societe->getType(), $societe->getRaisonSociale(), null,$societe->getAdresse()->getAdresse(), $societe->getAdresse()->getCodePostal(), $societe->getAdresse()->getCommune(), $societe->getSiret(), $societe->getCodeComptable(), $societe->getFrequencePaiement(), $societe->getContactCoordonnee()->getTelephoneFixe(), $societe->getContactCoordonnee()->getTelephoneMobile(), $societe->getContactCoordonnee()->getEmail(), $societe->getContactCoordonnee()->getSiteInternet(), $societe->getContactCoordonnee()->getLibelle(), $societe->getProvenance()), ';');
                foreach($societe->getEtablissements() as $etablissement) {
