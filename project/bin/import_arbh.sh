...
mongoimport /tmp/ahrb/Configuration.json -d $MONGODATABASE -c Configuration -h $MONGOHOST --drop
...
xlsx2csv -d ";" /tmp/ahrb/GESTION\ DES\ CONTRATS\ AHRB\ 2022.xlsx | grep -v "^;" | sed -r 's/^([^;]*);[^;]*;/\1;\1;/' | sed -r -f /tmp/ahrb/societes_contrats.sed | sed -r -f /tmp/ahrb/prestations_contrats.sed > /tmp/ahrb/contrats.csv
...
php5 app/console fos:elastica:reset;
php5 app/console fos:elastica:populate;